...
class ViewController: UIViewController {
    ...
    private func start() {
        // Создадим несколько границ для подбора паролей
        let borders: [(left: String, right: String)] = [
            ("0000", "dZZZ"),
            ("e000", "sZZZ"),
            ("t000", "HZZZ"),
            ("I000", "ZZZZ")
        ...
        // Для каждой из границ добавим задачу и начнём её асинхронное выполнение
        for border in borders {
            DispatchQueue.global(qos: .utility).async {
                let result = self.bruteForce(startString: border.left, endString: border.right)
                if let result = result {
        // Если ппароль родобрался, обновим UI
